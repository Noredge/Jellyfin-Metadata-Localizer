...
/// <summary>Groq transport only. Credentials are resolved on demand and never stored in the task snapshot.</summary>
public sealed class GroqTitleProvider : ITitleProvider, IDisposable
{
    ...
    public GroqTitleProvider(Func<CancellationToken, ValueTask<string>> credential, HttpMessageHandler? testHandler = null)
    {
    ...
    public async Task<TitleProviderResult> TranslateAsync(TitleProviderRequest request, CancellationToken token)
    {
        ...
        try
        {
            if (request.Spec.Endpoint != TitlePipeline.GroqEndpoint || string.IsNullOrWhiteSpace(request.Spec.Model)) throw new ArgumentException();
            ...
            var allowed = new HashSet<string>(StringComparer.Ordinal) { "temperature", "max_completion_tokens", "response_format", "reasoning_effort" };
        ...
        using var message = new HttpRequestMessage(HttpMethod.Post, TitlePipeline.GroqEndpoint + "/chat/completions");
    ...
    public async Task<TranslationModelInfo[]> ListModelsAsync(CancellationToken token)
    {
        ...
        using var message = new HttpRequestMessage(HttpMethod.Get, TitlePipeline.GroqEndpoint + "/models");
