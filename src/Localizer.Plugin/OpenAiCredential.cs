...
internal static class OpenAiCredential
{
    internal static string FilePath(string root) => Path.Combine(root, "credentials", "openai-api-key.dpapi");
    internal static string Load(string root)
    {
        ...
        try
        {
            ...
            try
            {
                ...
                if (!key.StartsWith("sk-", StringComparison.Ordinal) || key.Any(char.IsWhiteSpace) || key.Length > 4096)
